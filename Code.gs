function counter (range) {
  ...
  for (var i = 0; i < range.length; i++) {
    if (range[i][0] !== undefined && range[i][0] !== '' && range[i][0] !== null && range[i][0].toLowerCase() !== 'client advisor') {
      found = true;
    }
  }
  ...
  for (i = 0; i < values.length; i++) {
    if (values[i][0] !== undefined && values[i][0] !== null && values[i][0] !== '' && values[i][0].toLowerCase() !== 'lsp') {
      if (agents.length === 0) {
        ...
        if (values[i][2].toLowerCase() === 'yes') { agents[0][2] ++; }
        else if (values[i][2].toLowerCase() === 'no') { agents[0][3] ++; }
      }
      else {
        found = false;
        for (var j = 0; j < agents.length; j++) {
          if (values[i][0].toLowerCase() == agents[j][0]) {
            found = true;
            agents[j][1] ++;
            if (values[i][2].toLowerCase() === 'yes') { agents[j][2] ++; }
            else if (values[i][2].toLowerCase() === 'no') { agents[j][3] ++; }
          }
        }
        if (!found) {
          agents[agents.length] = [values[i][0].toLowerCase(), 1, 0, 0];
          if (values[i][2].toLowerCase() === 'yes') { agents[agents.length-1][2] ++; }
          else if (values[i][2].toLowerCase() === 'no') { agents[agents.length-1][3] ++; }
        }
      }
    }
  }
